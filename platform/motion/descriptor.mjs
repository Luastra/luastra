...
export function normalizeTweenDescriptor(value) {
  ...
  if (!Number.isFinite(value.durationMs) || value.durationMs < 0 || value.durationMs > 60_000) fail("motion durationMs must be between 0 and 60000");
  if (!easingNames.has(value.easing)) fail(`unsupported motion easing: ${value.easing}`);
  return Object.freeze({ kind: "tween", from: value.from, to: value.to, durationMs: value.durationMs, easing: value.easing });
}
