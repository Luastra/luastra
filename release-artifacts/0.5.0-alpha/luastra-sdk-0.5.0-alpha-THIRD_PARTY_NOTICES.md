...
This notice bundle applies to the downloadable Luastra SDK archives shipped
with the release that contains it. The archives contain Luastra project code,
the Apache-2.0 `universal-blocks` composition library, starter templates, Luau
native developer tools, and the Emscripten-generated Luau WebAssembly runtime.

`universal-blocks` contains no bundled third-party code or assets. Its own
license and notice are retained with the library source inside every SDK archive.
