...
export class EventFrameScheduler {
  ...
  cancel(id) {
    ...
    if (this.#tasks.size === 0 && this.#frameHandle !== null) {
  ...
  #runFrame(timestamp) {
    ...
    this.#frameHandle = null;
    const snapshot = [...this.#tasks.entries()];
    for (const [id, callback] of snapshot) {
      if (!this.#tasks.has(id)) continue;
      try {
        if (callback(timestamp) !== true) this.#tasks.delete(id);
      } catch (error) {
