...
const propertyRanges = Object.freeze({
  opacity: [0, 1],
...
function validateChannel(target, property, descriptor) {
  ...
  if (!range) fail(`unsupported motion property: ${property}`);
  const tweens = descriptor.kind === "tween" ? [descriptor] : descriptor.steps.filter((step) => step.kind === "tween");
  for (const tween of tweens) for (const [name, value] of [["from", tween.from], ["to", tween.to]]) {
    if (value < range[0] || value > range[1]) fail(`${property} ${name} is outside the supported range`);
  }
}
...
export class MotionRuntime {
  ...
  constructor({ scheduler, adapter, reducedMotion = () => false } = {}) {
    ...
    this.#engine = new TweenEngine(scheduler, { reducedMotion });
  }
  ...
  prepare(target, property, value) {
    ...
    const descriptor = normalizeMotionDescriptor(value);
    validateChannel(target, property, descriptor);
    const key = `${target}\u0000${property}`;
    const previous = this.#active.get(key);
    if (previous) this.#cancelRecord(previous, "replaced");
    const reduced = this.#reducedMotion();
    const endpoints = motionDescriptorEndpoints(descriptor);
  ...
  start(target, property, value, { onComplete = () => {}, onCancel = () => {} } = {}) {
    ...
    const record = { key, target, property, state: "active", reason: null, tween: null, onCancel };
    ...
    const apply = (next, progress) => {
      try { this.#adapter.applyMotionValue(target, property, next, progress); }
      catch (error) { if (this.#active.get(key) === record) this.#active.delete(key); record.state = "failed"; throw error; }
    };
    ...
    return Object.freeze({
      ...
      cancel: () => this.#cancelRecord(record, "cancelled"),
    });
  }
  ...
  disposeTarget(target) {
    ...
    for (const record of [...this.#active.values()]) {
      if (record.target === target && this.#cancelRecord(record, "target-removed")) cancelled += 1;
    }
    this.#adapter.clearMotionTarget?.(target);
    return cancelled;
  }

  dispose() {
    if (this.#disposed) return false;
    for (const record of [...this.#active.values()]) this.#cancelRecord(record, "session-disposed");
    this.#disposed = true;
    return true;
  }
  ...
  #startSequence(record, descriptor, apply, complete) {
    ...
    let current = motionDescriptorEndpoints(descriptor).from;
    const advance = () => {
      ...
      const stepHandle = this.#engine.animate({
        ...tween,
  ...
  #cancelRecord(record, reason) {
    if (record.state !== "active" || this.#active.get(record.key) !== record) return false;
